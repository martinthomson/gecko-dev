...
ssl3_HandleServerNameXtn(sslSocket * ss, PRUint16 ex_type, SECItem *data)
{
    ...
    if (listLenBytes < 0) {
        return SECFailure; /* a fatal alert has been sent */
    }
    if (listLenBytes == 0 || listLenBytes != data->len) {
        return SECSuccess; /* ignore an empty or malformed extension */
    }
    ldata = *data;
    ...
    if (!listCount) {
        return SECSuccess;  /* nothing we can act on */
    }
...
static SECStatus
ssl3_PerformAppProtocolSelection(sslSocket *ss, PRUint16 ex_type, SECItem *data,
                                 PRBool isNPN) {
    SECStatus rv;
    ...
    if (!isNPN && ss->ssl3.nextProtoState != SSL_NEXT_PROTO_NEGOTIATED) {
        /* The handler might say OK, but then it's picked a default.
...
ssl3_ServerHandleAppProtoXtn(sslSocket *ss, PRUint16 ex_type, SECItem *data)
{
    ...
    rv = ssl3_PerformAppProtocolSelection(ss, ex_type, data, PR_FALSE);
    if (rv != SECSuccess) {
      return rv;
    }
                                  ...
                                  SECItem *data)
{
    ...
    return ssl3_PerformAppProtocolSelection(ss, ex_type, data, PR_TRUE);
}
