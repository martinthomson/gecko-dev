...
function run_server_with_version(vmin, vmax, vintolerant) {
  ...
  if (typeof vintolerant === "number") {
    envSvc.set("TLS_SERVER_VERSION_INTOLERANT", vintolerant.toString(10));
  } else {
    // this clears the variable, effectively
    envSvc.set("TLS_SERVER_VERSION_INTOLERANT", "");
  }
...
function test_version_range(connectVersion, vmin, vmax, vfb) {
  ...
  return connect_to_host("intolerant.example.com",
...
function test_with_tls10_to_tls12() {
  ...
  yield test_version_range(-1, 0, 0); // SSL 3.0 only fails
...
function test_with_sslv3_only() {
  ...
  yield test_version_range(-1, 1, 3); // defaults fail
  yield test_version_range(0, 0, 0);  // SSL 3.0 only is OK
  yield test_version_range(-1, 3, 3); // TLS 1.2 only fails
...
function test_with_tls12_only() {
  ...
  yield test_version_range(-1, 0, 0);  // SSL 3.0 only is OK
...
function test_intolerant_fallback_ssl3() {
  // configure the server to attempt to handle all versions,
  // but choke on TLS 1.0 (1) or higher
  yield run_server_with_version(0, 3, 1);

  yield test_version_range(0, 0, 3, 0); // full range, permit fallback
