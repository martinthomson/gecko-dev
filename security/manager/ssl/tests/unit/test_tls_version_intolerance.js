...
function collect_fallback_telemetry() {
  return [
    { pre: "SSL_SSL30_INTOLERANCE_REASON_PRE",
      post: "SSL_SSL30_INTOLERANCE_REASON_POST" },
...
function *run_tls_fallback_scsv_test() {
  // This server has forced intolerance at TLS 1.0, but it is configured for TLS
  // 1.2. When we connect, we fallback to SSLv3, which then triggers an
...
function *run_tls_intolerance_test() {
  // this server has forced intolerance at TLS 1.0, period
  yield run_server_with_version(0, 1, 1);
...
function *run_tls_fallback_pref_test() {
  ...
  ++fallbackBaseline[2].pre; // no post because we block fallback
