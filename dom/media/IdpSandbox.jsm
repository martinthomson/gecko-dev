...
IdpSandbox.prototype = {
  /**
   * Get a promise containing the IdP proxy object.
   */
  get idp() {
    return this.active;
  },
  ...
  _createSandbox: function(result) {
    ...
    this.sandbox = Cu.Sandbox(principal, {
      ...
      wantGlobalProperties: [
        ...
        'URL', 'URLSearchParams', 'atob', 'btoa', 'Blob'
      ]
    });
    this._populateSandbox();

    let resolveRegistration;
    let idpRegistered = new Promise(resolve => {
  ...
  toString: function() {
    return this.source.spec + ' [' + (this.registered ? '' : 'not ') + 'ready]';
  }
};
