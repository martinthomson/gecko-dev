...
/**
 * An invisible iframe for hosting the IdP shim.  This does the real work on the
 * parent process side of things, loading the IdP iframe and dealing with its
 * message channel.
 *
 ...
 * the RTC functions).
 */
function IdpSandbox(uri, messageCallback) {
  this.sandbox = null;
...
/**
 * Parent process side, this class listens from messages from the content
 * process and creates IdpWrapper instances as requested.  It manages the
 * lifecycle of the IdpWrappers, and maps messages from content processes into
 * function calls on the wrappers.
 */
  ...
  CREATE: function(caller, id, url) {
    ...
    wrapper.open(e => {
      if (e) {
        ...
        // We don't want a strong reference; that would keep the wrapper
        // alive.  Better to let the normal lifecycle run its course.
