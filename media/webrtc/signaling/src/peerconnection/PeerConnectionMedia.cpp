...
void
PeerConnectionMedia::UpdateSinkPrincipal_m(nsIPrincipal* aPrincipal)
{
  ASSERT_ON_THREAD(mMainThread);

  for (uint32_t u = 0; u < mLocalSourceStreams.Length(); u++) {
    mLocalSourceStreams[u]->UpdateSinkPrincipal_m(aPrincipal);
  }
}
...
LocalSourceStreamInfo::UpdateSinkPrincipal_m(nsIPrincipal* aPrincipal)
{
  for (auto it = mPipelines.begin(); it != mPipelines.end(); ++it) {
    ...
    pipeline->UpdateSinkPrincipal_m(aPrincipal);
  }
}
...
{
  mMediaStream->CombineWithPrincipal(aPrincipal);
}
