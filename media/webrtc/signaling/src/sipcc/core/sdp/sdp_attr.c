                                 ...
                                 const char *ptr)
{
    ...
    ptr = sdp_getnextstrtok(ptr, attr_p->attr.msid.identity,
                            sizeof(attr_p->attr.msid.identity), " \t", &result);
    if (result != SDP_SUCCESS) {
        sdp_parse_error(sdp_p->peerconnection,
    ...
    if (sdp_p->debug_flag[SDP_DEBUG_TRACE]) {
        SDP_PRINT("%s Parsed a=msid, %s %s", sdp_p->debug_str,
                  attr_p->attr.msid.identity, attr_p->attr.msid.appdata);
    }
